...
int EuroToDollar3() {
  EuroToDollarConverter converter;
  TINYTEST_EQUAL_EPSILON(converter.convert(42.0),46.317433); // for some reason this fails, even though the numbers are equal
  return 1;
}
...
int DollarToYen2() {
  DollarToYenConverter converter;
  TINYTEST_EQUAL_EPSILON(-120.684499,converter.convert(-1.0));
  return 1;
}
int DollarToYen3() {
  DollarToYenConverter converter;
  TINYTEST_EQUAL_EPSILON(5068.748958,converter.convert(42.0));
  return 1;
}
...
int InchesToCentimeter1() {
  InchToCentimetersConverter converter;
  TINYTEST_EQUAL_EPSILON(0.0,converter.convert(0.0));
  return 1;
}
...
int FahrenheitToCelsius1() {
  ...
  TINYTEST_EQUAL_EPSILON(-17.777778,converter.convert(0.0)); // for some reason this fails, even though the numbers are equal
  return 1;
}
int FahrenheitToCelsius2() {
  FahrenheitToCelsiusConverter converter;
  TINYTEST_EQUAL_EPSILON(-18.333333,converter.convert(-1.0)); // for some reason this fails, even though the numbers are equal
  return 1;
}
int FahrenheitToCelsius3() {
  FahrenheitToCelsiusConverter converter;
  TINYTEST_EQUAL_EPSILON(5.555556,converter.convert(42.0)); // for some reason this fails, even though the numbers are equal
  return 1;
}
